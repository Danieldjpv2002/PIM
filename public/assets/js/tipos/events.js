...
const onButtonUpdateClicked = async (data) => {
    const modal = $('#modal-tipos')
    if (data?.id) modal.find('.modal-title').text('Editar tipo')
    else modal.find('.modal-title').text('Nuevo tipo')

    txt_id.value = data?.id ?? ''
    cbo_categoria.value = data?.categoria?.id ?? ''
    $(cbo_categoria).select2({
        dropdownParent: '#modal-tipos'
    })
    txt_tipo.value = data?.tipo ?? ''
